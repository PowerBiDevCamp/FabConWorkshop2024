...
public class FabricUserApi {
  ...
  public static FabricItem GetSemanticModelByName(string WorkspaceId, string Name) {
    string jsonItemsList = ExecuteGetRequest("/workspaces/" + WorkspaceId + "/items/?type=SemanticModel");
    var items = JsonSerializer.Deserialize<FabricItemList>(jsonItemsList).value;
    foreach (var item in items) {
  ...
  public static FabricItem GetReportByName(string WorkspaceId, string Name) {
    string jsonItemsList = ExecuteGetRequest("/workspaces/" + WorkspaceId + "/items/?type=Report");
    var items = JsonSerializer.Deserialize<FabricItemList>(jsonItemsList).value;
    foreach (var item in items) {
      if (Name == item.displayName) {
        return item;
