...
public class FabricUserApi {
  ...
  public static LakehouseShortcut CreateLakehouseShortcut(string WorkspaceId, string LakehouseId, LakehouseShortcutCreateRequest shortcutCreateRequest) {

    string displayMessage = string.Format(" - Creating shortcut named {0}", shortcutCreateRequest.name);
    Console.Write(displayMessage);
  ...
  public static void RunDataPipeline(string WorkspaceId, FabricItem Item) {
    Console.Write(" - Running data pipeline named " + Item.displayName);

    string restUrl = "/workspaces/" + WorkspaceId + "/items/" + Item.id + "/jobs/instances?jobType=Pipeline";
    ExecutePostRequest(restUrl, "{ \"executionData\": {} }");

    Console.WriteLine("   > Data pipeline execution completed");
    Console.WriteLine();
  }
